...
void MatrixElements::calculateH(mat& A1, mat& A2, vec& s1, vec& s2, double& Hij, double& Bij){
  ...
  double overlap = (pow(datum::pi,3.0*n/2.0)/sqrt(detB)) * exp(-dot(s1,A1*s1) - dot(s2,A2*s2) + 1/4.0 *dot(v,Bi*v));
  double T = overlap*(2.0*trace(prod12*Bi) + 4.0*dot(u-s1,prod12*(u-s2)));
...
void MatrixElements::calculateH_noShift(mat& A1, mat& A2, double& Hij, double& Bij){
  ...
  double overlap = pow(datum::pi,3.0*n/2.0)/sqrt(detB);
  double T = overlap*(2.0*trace(prod12*Bi) );
