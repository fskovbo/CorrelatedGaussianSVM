...
double NumGradient(System& sys,MatrixElements& elem, mat& Afixed, mat& Avar, double eps){
...
int main() {
  ...
  size_t Npts = 1e2;
  mat data(Npts,5);
  vec Avals = linspace(1000,6000,Npts);
  vec** vArray;
  ...
  for (size_t i = 0; i < Npts; i++) {
    ...
    vec Alist = ones<vec>(De*Test.n*(Test.n+1)/2);

    Alist(variableNr) = Avals(i);
    ...
    elem.calculateH_noShift(Afixed,A,Hij,Bij,Hg,Bg);
    // mat B = A+A;
    // double detB = det(B);
    // mat Binv = B.i();
    ...
    data(i,2) = (Hg(variableNr)-Hij/Bij * Bg(variableNr))/Bij;
    // data(i,1) = Vstrat.calculateExpectedPotential_noShift(A,A,Binv,detB,Vgrad,Binvgrad,detBgrad);
    // data(i,2) = Vgrad(0);
    data(i,3) = NumGradient(Test,elem,Afixed,A,1e-1);
